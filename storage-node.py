...
def heartbeat_loop(node_name: str):
    url = f"{CONTROLLER_URL}/nodes/heartbeat"
    while True:
        try:
            requests.post(url, json={"node": node_name}, timeout=1)
...
hb_thread = threading.Thread(
    target=heartbeat_loop, args=(node_name,), daemon=True
)
hb_thread.start()
...
receiver.setsockopt(zmq.SUBSCRIBE, node_name.encode("utf-8"))
