...
def get_node_status():
    ...
    now = time.time()
    dead = [
        name for name, ts in NODE_LAST_SEEN.items()
...
def download_file(file_id):
    ...
    live_nodes, dead_nodes = get_node_status()
    ...
    nodes_lists = [
        f.get('part1_nodes', '').split(','),
        f.get('part2_nodes', '').split(','),
        f.get('part3_nodes', '').split(','),
        f.get('part4_nodes', '').split(',')
    ]
      
    # prevent void name
    part_lists = [[name for name in lst if name] for lst in part_lists]
    nodes_lists = [[n for n in lst if n] for lst in nodes_lists]

    chosen_names = []


    for frag_idx in range(4):
        names = part_lists[frag_idx]
        nodes = nodes_lists[frag_idx]

        if not names:
            continue   # skip empty fragment lists

        # Find live replicas for this fragment
        live_replicas = [
            name for name, node in zip(names, nodes)
            if node in live_nodes
        ]

        if not live_replicas:
            # No live replicas for this fragment
            return make_response(
                {"message": "File lost: fragment has no live replicas",
                 "dead_nodes": dead_nodes},
                500
            )
        # Randomly choose one live replica
        chosen_names.append(
            live_replicas[random.randint(0, len(live_replicas) - 1)]
        )
    print(f"Chosen chunk names (only live nodes): {chosen_names}")
...
def add_files():
    ...
    fragment_nodes_lists = []  # [[n11,n12,...,n1k], ..., [n41,...,n4k]]
    ...
    cursor = db.execute(
        ...
        "`part1_nodes`, `part2_nodes`, `part3_nodes`, `part4_nodes`) "
        "VALUES(?,?,?,?,?,?,?,?,?,?,?)",
        (
            ...
            ','.join(fragment_nodes_lists[0]),
            ','.join(fragment_nodes_lists[1]),
            ','.join(fragment_nodes_lists[2]),
            ','.join(fragment_nodes_lists[3]),
        )
    )
    db.commit()
...
def node_heartbeat():
    ...
    node = data.get("node")
    if node not in NODE_LAST_SEEN:
        return make_response({"message": f"Unknown node {node}"}, 400)

    NODE_LAST_SEEN[node] = time.time()
    return make_response({"status": "ok"})

@app.route("/services/loss_fraction", methods=["GET"])
def loss_fraction():
    live_nodes, dead_nodes = get_node_status()
    ...
    for f in files:
        ...
        for frag_idx in range(4):
            ...
            has_live = any(node in live_nodes for node in nodes)
            if not has_live:
                file_lost = True
                break
